...
import '../components/LanguageListElement.dart';

class LanguagePage extends StatefulWidget {
  LanguagePage({Key key, this.title, this.isAutomaticEnabled}): super(key:key);
...
class _LanguagePageState extends State<LanguagePage> {
  ...
  void initState() {
    ...
    if(!this.widget.isAutomaticEnabled) {
      var automaticElement = this._languageList
  ...
  Widget _displayListWithHeaders() {
    List<Language> recentLanguages = this._languageList.where((lang) => lang.isRecent).toList();
  ...
  Widget _displaySearchedList() {
    List<Language> searchedList = this._languageList
        .where((e) => e.name.toLowerCase().contains(this._searchTextController.text.toLowerCase()))
        .toList();
